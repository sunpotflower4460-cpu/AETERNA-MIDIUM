...
describe('ReplayQueue', () => {
  ...
  it('should add salient candidates', () => {
    const queue = new ReplayQueue();
    queue.addCandidate('touch', 0.8, 100);
    expect(queue.size()).toBe(1);
  });

  it('should reject low-salience candidates', () => {
    const queue = new ReplayQueue();
    queue.addCandidate('touch', 0.2, 100); // Below threshold
    expect(queue.size()).toBe(0);
  });

  it('should prioritize high-weight candidates', () => {
    const queue = new ReplayQueue();
    queue.addCandidate('touch', 0.5, 100);
    queue.addCandidate('surprise', 0.9, 110);
    queue.addCandidate('restoration', 0.6, 120);

    const candidates = queue.getCandidatesForReplay(2);
    expect(candidates.length).toBe(2);
    expect(candidates[0].category).toBe('surprise'); // Highest salience
  });
  ...
  it('should respect max capacity', () => {
    ...
    for (let i = 0; i < 20; i++) {
      queue.addCandidate('touch', 0.5, i);
    }
...
describe('deriveReplayState', () => {
  ...
  it('should derive replay pressure in quiet conditions', () => {
    ...
    const queue = new ReplayQueue();
    queue.addCandidate('touch', 0.7, 100);
...
describe('Replay Integration', () => {
  it('should maintain bounded values under extreme conditions', () => {
    ...
    // Add many candidates
    for (let i = 0; i < 100; i++) {
      queue.addCandidate('touch', Math.random(), i);
    }
